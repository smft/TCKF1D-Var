...
for i in range(31):
    cell_st_datetime=start_date+timedelta(days=i)
    cell_ed_datetime=start_date+timedelta(days=i+1)
    for station_id in gmwr_station_location.keys():
        if station_id=="54340":
            station_id_meteostats="54342"
        elif station_id=="54727":
            station_id_meteostats="54823"
        else:
            station_id_meteostats=station_id

        data=(Hourly(station_id_meteostats,\
                     datetime(cell_st_datetime.year,\
                              cell_st_datetime.month,\
                              cell_st_datetime.day,\
                              cell_st_datetime.hour),\
                     datetime(cell_ed_datetime.year,\
                              cell_ed_datetime.month,\
                              cell_ed_datetime.day,\
                              cell_ed_datetime.hour)).fetch()).to_numpy()
        for hour,cell_data in enumerate(data):
            if cell_data[3]>=10:
                print(station_id,cell_st_datetime+timedelta(hours=hour),cell_data[3])