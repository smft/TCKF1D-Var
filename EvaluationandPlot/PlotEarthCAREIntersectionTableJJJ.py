...
for line in open("/root/Scripts/IntersectionList.txt","r").read().split("\n"):
    for idx in line.split(",")[1:-1]:
        station_id=list(gmwr_station_location.keys())[int(idx)]
        gmwr_station_location[station_id][-1]+=1
        print("time ./MainJJJ.py "+station_id+" "+line.split(",")[-1])

total_amount=0
for station_id in gmwr_station_location.keys():
    if gmwr_station_location[station_id][-1]!=0:
        string=station_id+","+\
               str(np.round(gmwr_station_location[station_id][0],2))+","+\
               str(np.round(gmwr_station_location[station_id][1],2))+","+\
               str(gmwr_station_location[station_id][3])
        total_amount+=gmwr_station_location[station_id][3]
        print(string)
print(total_amount)