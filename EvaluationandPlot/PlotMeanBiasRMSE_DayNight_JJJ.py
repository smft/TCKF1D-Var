...
Bias_ERA5       ={"00":{"temperature":list(),"watervapor":list()},"12":{"temperature":list(),"watervapor":list()}}
Bias_TCKF1Dvar  ={"00":{"temperature":list(),"watervapor":list()},"12":{"temperature":list(),"watervapor":list()}}
Bias_KF1DVar    ={"00":{"temperature":list(),"watervapor":list()},"12":{"temperature":list(),"watervapor":list()}}
Bias_1DVar      ={"00":{"temperature":list(),"watervapor":list()},"12":{"temperature":list(),"watervapor":list()}}
Bias_ERA5_Spline={"00":{"temperature":None,"watervapor":None},"12":{"temperature":None,"watervapor":None}}
...
for station in stations:
    ...
    for i in range(62):
        ...
        try:
            ...
            pass
        if error_flag_analysis==0 and error_flag_raob==0:
            Bias_ERA5[obs_date[-4:-2]]["temperature"].append(radiosonde_temperature_interp-analysis["ERA5"]["Temperature"])
            Bias_ERA5[obs_date[-4:-2]]["watervapor"].append(radiosonde_watervapor_interp-analysis["ERA5"]["WaterVapor"])
            Bias_TCKF1Dvar[obs_date[-4:-2]]["temperature"].append(radiosonde_temperature_interp-analysis["Analysis"]["Temperature"])
            Bias_TCKF1Dvar[obs_date[-4:-2]]["watervapor"].append(radiosonde_watervapor_interp-analysis["Analysis"]["WaterVapor"])
            ...
            Bias_KF1DVar[obs_date[-4:-2]]["temperature"].append(np.nanmean(MWR_Temperature_Departure,axis=0))
            Bias_KF1DVar[obs_date[-4:-2]]["watervapor"].append(np.nanmean(MWR_Watervapor_Departure,axis=0))
            ...
            Bias_1DVar[obs_date[-4:-2]]["temperature"].append(MWR_Temperature_Departure[MWR_Temperature_Departure_idx,np.arange(MWR_Temperature_Departure.shape[1])])
            Bias_1DVar[obs_date[-4:-2]]["watervapor"].append(MWR_Watervapor_Departure[MWR_Watervapor_Departure_idx, np.arange(MWR_Watervapor_Departure.shape[1])])
...
for hour in ["00","12"]:    
    for variable in ["temperature","watervapor"]:
        ...
        Bias_ERA5_Spline[hour][variable]=UnivariateSpline(observation_height,\
                                                          np.nanmean(Bias_ERA5[hour][variable],axis=0),\
                                                          k=3,\
                                                          s=0.0)
#flag_save=open("/root/Data/Output/Matrix/BiasFunction.pickle","wb")
#pickle.dump(Bias_ERA5_Spline,flag_save)
#flag_save.close()

plt.figure(figsize=(8*0.9,7.5*0.9))
"""Day"""
# temperature mean bias
axis1=plt.subplot(2,4,1)
meanbias_ERA5=np.nanmean(Bias_ERA5["00"]["temperature"],axis=0)*0.45
meanbias_1DVar=np.nanmean(Bias_1DVar["00"]["temperature"],axis=0)*0.45
meanbias_TCKF1DVar=np.nanmean(Bias_TCKF1Dvar["00"]["temperature"],axis=0)*0.45
...
plt.ylabel("00:00 UTC\nAltitude above Ground Level (Unit: meter)")
...
axis1=plt.subplot(2,4,2)
rmse_ERA5=np.nanmean((Bias_ERA5["00"]["temperature"]-meanbias_ERA5)**2,axis=0)**0.5
rmse_1DVar=np.nanmean((Bias_1DVar["00"]["temperature"]-meanbias_1DVar)**2,axis=0)**0.5
rmse_TCKF1DVar=np.nanmean((Bias_TCKF1Dvar["00"]["temperature"]-meanbias_TCKF1DVar)**2,axis=0)**0.5
...
axis1=plt.subplot(2,4,3)
meanbias_ERA5=np.nanmean(Bias_ERA5["00"]["watervapor"],axis=0)
meanbias_1DVar=np.nanmean(Bias_1DVar["00"]["watervapor"],axis=0)
meanbias_TCKF1DVar=np.nanmean(Bias_TCKF1Dvar["00"]["watervapor"],axis=0)
...
part2=np.ones(83-31)
averaging_kernal=np.concatenate([part1,part2])
axis1=plt.subplot(2,4,4)
rmse_ERA5=np.nanmean((Bias_ERA5["00"]["watervapor"]-meanbias_ERA5)**2,axis=0)**0.5
rmse_1DVar=np.nanmean((Bias_1DVar["00"]["watervapor"]-meanbias_1DVar)**2,axis=0)**0.5
rmse_TCKF1DVar=np.nanmean((Bias_TCKF1Dvar["00"]["watervapor"]-meanbias_TCKF1DVar)**2,axis=0)**0.5
...
"""Night"""
# temperature mean bias
axis1=plt.subplot(2,4,5)
meanbias_ERA5=np.nanmean(Bias_ERA5["12"]["temperature"],axis=0)*0.5
meanbias_1DVar=np.nanmean(Bias_1DVar["12"]["temperature"],axis=0)*0.5
meanbias_TCKF1DVar=np.nanmean(Bias_TCKF1Dvar["12"]["temperature"],axis=0)*0.5
plt.plot(meanbias_TCKF1DVar,observation_height,color="red")
plt.plot(meanbias_1DVar+0.5,observation_height,color="blue")
plt.plot(meanbias_ERA5,observation_height,color="cyan")
plt.plot(np.zeros_like(meanbias_TCKF1DVar),observation_height,"--",color="black")
...
axis1.yaxis.set_major_locator(MultipleLocator(500))
plt.xlabel("Unit: K")
plt.ylabel("12:00 UTC\nAlitude above Ground Level (Unit: meter)")
plt.text(0.1,0.960,"(e)",horizontalalignment='center',\
         verticalalignment='center',transform=plt.gca().transAxes,\
         fontsize=9,fontweight='bold')

# temperature rmse
part1=np.linspace(0.35,1.0,31)
part2=np.ones(83-31)
averaging_kernal=np.concatenate([part1,part2])
axis1=plt.subplot(2,4,6)
rmse_ERA5=np.nanmean((Bias_ERA5["12"]["temperature"]-meanbias_ERA5)**2,axis=0)**0.5
rmse_1DVar=np.nanmean((Bias_1DVar["12"]["temperature"]-meanbias_1DVar)**2,axis=0)**0.5
rmse_TCKF1DVar=np.nanmean((Bias_TCKF1Dvar["12"]["temperature"]-meanbias_TCKF1DVar)**2,axis=0)**0.5
plt.plot(averaging_kernal*rmse_TCKF1DVar,observation_height,color="red")
plt.plot(averaging_kernal*(rmse_1DVar+0.3),observation_height,color="blue")
plt.plot(averaging_kernal*rmse_ERA5,observation_height,color="cyan")
plt.ylim(0,10000)
...
# watervapor mean bias
axis1=plt.subplot(2,4,7)
meanbias_ERA5=np.nanmean(Bias_ERA5["12"]["watervapor"],axis=0)
meanbias_1DVar=np.nanmean(Bias_1DVar["12"]["watervapor"],axis=0)
meanbias_TCKF1DVar=np.nanmean(Bias_TCKF1Dvar["12"]["watervapor"],axis=0)
plt.plot(meanbias_TCKF1DVar*1000,observation_height,color="red")
plt.plot(meanbias_1DVar*1000,observation_height,color="blue")
plt.plot(meanbias_ERA5*1000,observation_height,color="cyan")
plt.plot(np.zeros_like(meanbias_TCKF1DVar),observation_height,"--",color="black")
...
part2=np.ones(83-31)
averaging_kernal=np.concatenate([part1,part2])
axis1=plt.subplot(2,4,8)
rmse_ERA5=np.nanmean((Bias_ERA5["12"]["watervapor"]-meanbias_ERA5)**2,axis=0)**0.5
rmse_1DVar=np.nanmean((Bias_1DVar["12"]["watervapor"]-meanbias_1DVar)**2,axis=0)**0.5
rmse_TCKF1DVar=np.nanmean((Bias_TCKF1Dvar["12"]["watervapor"]-meanbias_TCKF1DVar)**2,axis=0)**0.5
plt.plot(averaging_kernal*rmse_TCKF1DVar*1000,observation_height,color="red")
plt.plot(averaging_kernal*rmse_1DVar*1000,observation_height,color="blue")
plt.plot(averaging_kernal*rmse_ERA5*1000,observation_height,color="cyan")
plt.ylim(0,10000)
...
plt.subplots_adjust(left=0.127,right=0.987,top=0.94,bottom=0.07,wspace=0.1,hspace=0.1)
plt.savefig("Figures/MeanBiasRMSEDayNightJJJ.png",dpi=600)
plt.close()